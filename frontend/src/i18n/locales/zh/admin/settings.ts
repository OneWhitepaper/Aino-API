    ...
    settings: {
      desktop: {
        ...
        import: {
          open: '自动获取模型',
          title: '从现有来源批量导入',
          description: '选择分组和来源账号，通过已配置的接口获取模型，无需重新填写密钥。获取操作也会更新来源账号的模型能力缓存。',
          account: '来源账号',
          noAccounts: '此分组暂无可用来源账号，请先在账号管理中关联账号，或继续手动添加。',
          accountsFailed: '来源账号加载失败，请重试。',
          fetch: '获取模型列表',
          fetching: '正在获取…',
          fetchFailed: '获取失败。请检查来源账号、网络及上游是否支持模型列表接口，然后重试；仍可手动添加。',
          metadataWarning: '部分模型资料不完整或接口使用了已配置模型作为候选，请检查导入结果并补充缺失信息。',
          search: '搜索待导入模型',
          selectAll: '全选筛选结果',
          clearSelection: '清空选择',
          count: '获取 {total} 个，已选 {selected} 个，还可添加 {remaining} 个',
          reviewHint: '自动填写模型 ID、名称及已返回的能力参数。重复项自动跳过，价格沿用现有分组；工具调用能力和 Agent 验证需实际确认，新增模型默认待验证。',
          limit: '目录最多 200 个模型，当前还可添加 {remaining} 个，请减少选择。',
          confirm: '导入选中模型（{count}）',
          success: '已添加 {count} 个模型到当前表单，请核对后保存设置。',
          exists: '已添加到该分组和协议',
          notAllowed: '不在所选分组白名单内',
        },
        title: 'Aino 内置模型',
